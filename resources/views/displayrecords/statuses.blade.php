...
<div class="container">
    ...
    <div class="row">
            <div class="col-auto">
                    ...
                    'method' => 'GET',
                    'class'=>"form-inline",                                          
                    'action'=>'DisplayRecordsController@displayStatuses'
                    ])!!}
                    @csrf
                    ...
                    <a href="{{route('displayrecords.statuses')}}">Reset</a>
                </div>
                   <div class="col-auto text-right">
        <a href="{{route('displayrecords.statuses',['transaction_date'=>request('transaction_date'),'sort'=>'asc'])}}">Ascending</a>
        <a href="{{route('displayrecords.statuses',['transaction_date'=>request('transaction_date'),'sort'=>'desc'])}}">Descending</a>
        </div>
    </div>
    <div class="row">   
        <div class="form-row align-items-center">            
            ...
            'method' => 'GET',
            'class'=>"form-inline",                                          
            'action'=>'ExcelController@exportStatuses'
            ])!!}
            @csrf
...
<thead>
    ...
    <tbody>
        ...
        <tr>
                <td>  {!! Form::open([
                        'method' => 'DELETE',
                        'action' => ['DestroyController@destroyStatuses', $status->id]
                    ]) !!}
                        {!! Form::submit('Delete', ['class' => 'btn btn-danger']) !!}
                    {!! Form::close() !!}
                </td>
        <td><a href="{{ action('EditController@editStatuses', $status->id) }}" style="color: black">{{$status->id}}</a></td>
        <td><a href="{{ action('EditController@editStatuses', $status->id) }}" style="color: black">{{$status->description}}</a></td>
        <td><a href="{{ action('EditController@editStatuses', $status->id) }}" style="color: black">{{$status->created_by}}</a></td>            
        <td><a href="{{ action('EditController@editStatuses', $status->id) }}" style="color: black">{{$status->created_at}}</a></td>
        <td><a href="{{ action('EditController@editStatuses', $status->id) }}" style="color: black">{{$status->updated_at}}</a></td>
        </tr>
        @endforeach
    </tbody>
</thead>
